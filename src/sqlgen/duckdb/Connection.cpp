...
typename Connection::ConnPtr Connection::make_conn(
  ...
  if (PQstatus(raw_ptr) != CONNECTION_OK) {
    const auto msg =
        std::string("Connection to duckdb failed: ") + PQerrorMessage(raw_ptr);
    PQfinish(raw_ptr);
...
Result<Ref<IteratorBase>> Connection::read(const dynamic::SelectFrom& _query) {
  const auto sql = duckdb::to_sql_impl(_query);
  try {
...
Result<Nothing> Connection::write(
  ...
  for (const auto& line : _data) {
    ...
    if (success != 1) {
      ...
      return error("Error occurred while writing data to duckdb.");
    }
  }
  return Nothing{};
}
