...
TEST(sqlite, test_range_select_from_with_timestamps) {
  ...
  const std::string expected_query =
      R"(SELECT datetime("birthday", '+10 days') AS "birthday", julianday('2011-01-01') - julianday("birthday") AS "age_in_days", unixepoch(datetime("birthday", '+10 days'), 'subsec') AS "birthday_unixepoch" FROM "Person" ORDER BY "id";)";
