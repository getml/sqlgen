...
TEST(postgres, test_create_view_as) {
  ...
  const auto get_names = create_view_as<Name>(names_query);

  std::cout << postgres::to_sql(get_names) << std::endl;
  ...
  conn.and_then(drop_view<Name>).value();

  const std::string expected_query =
      R"(CREATE VIEW "Name" AS SELECT "first_name", "last_name" FROM "Person")";
