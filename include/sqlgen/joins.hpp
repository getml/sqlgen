...
template <rfl::internal::StringLiteral _alias, class TableOrQueryType,
          class ConditionType>
auto join(const transpilation::JoinType _how,
          const TableOrQueryType& _table_or_query, const ConditionType& _on) {
  return transpilation::Join<TableOrQueryType, ConditionType, _alias>{
      .how = _how, .table_or_query = _table_or_query, .on = _on};
};
